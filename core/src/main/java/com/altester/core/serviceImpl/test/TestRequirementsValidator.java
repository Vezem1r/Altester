...
public class TestRequirementsValidator {
    ...
    /**
     * Verifies if a test meets all the difficulty requirements.
     * A test meets the requirements if it has at least the specified number
    ...
    public boolean requirementsMet(Test test) {
        ...
        if (test.getEasyQuestionsCount() > 0 &&
                (questionCounts.getOrDefault(QuestionDifficulty.EASY, 0L) < test.getEasyQuestionsCount())) {
            log.debug("Test ID {} does not have enough EASY questions. Required: {}, Available: {}",
                    test.getId(), test.getEasyQuestionsCount(),
                    questionCounts.getOrDefault(QuestionDifficulty.EASY, 0L));
            return false;
        }

        if (test.getMediumQuestionsCount() > 0 &&
                (questionCounts.getOrDefault(QuestionDifficulty.MEDIUM, 0L) < test.getMediumQuestionsCount())) {
            log.debug("Test ID {} does not have enough MEDIUM questions. Required: {}, Available: {}",
                    test.getId(), test.getMediumQuestionsCount(),
                    questionCounts.getOrDefault(QuestionDifficulty.MEDIUM, 0L));
            return false;
        }

        if (test.getHardQuestionsCount() > 0 &&
                (questionCounts.getOrDefault(QuestionDifficulty.HARD, 0L) < test.getHardQuestionsCount())) {
            log.debug("Test ID {} does not have enough HARD questions. Required: {}, Available: {}",
                    test.getId(), test.getHardQuestionsCount(),
    ...
    public String getMissingRequirements(Test test) {
        ...
        if (test.getEasyQuestionsCount() > 0 &&
                (questionCounts.getOrDefault(QuestionDifficulty.EASY, 0L) < test.getEasyQuestionsCount())) {
            message.append(String.format("Needs %d EASY questions (has %d)",
        ...
        if (test.getMediumQuestionsCount() > 0 &&
            ...
            message.append(String.format("Needs %d MEDIUM questions (has %d)",
        ...
        if (test.getHardQuestionsCount() > 0 &&
                (questionCounts.getOrDefault(QuestionDifficulty.HARD, 0L) < test.getHardQuestionsCount())) {
            if (hasIssues) message.append(", ");
            message.append(String.format("Needs %d HARD questions (has %d)",
