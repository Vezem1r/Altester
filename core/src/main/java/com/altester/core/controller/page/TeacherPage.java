...
public class TeacherPage {
    ...
    public ResponseEntity<Page<TeacherStudentsDTO>> getTeacherStudents(
        ...
        Page<TeacherStudentsDTO> students = teacherPageService.getStudents(principal, page, size, search);
    ...
    public ResponseEntity<Page<ListTeacherGroupDTO>> getTeacherGroups(
            ...
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(required = false) String search,
            @RequestParam(required = false) @Pattern(regexp = "^(active|inactive|future)?$",
                    message = "Status must be one of: active, inactive, future, or empty") String status,
            Principal principal) {

        log.debug("Fetching groups: search: {}, status: {}", search, status);

        Page<ListTeacherGroupDTO> groups = teacherPageService.getGroups(
                principal, page, size, search, status);
    ...
    public ResponseEntity<String> moveStudentToAnotherGroup(
        ...
        log.debug("Moving student {} from group {} to group {}",
                request.getStudentUsername(), request.getFromGroupId(),
