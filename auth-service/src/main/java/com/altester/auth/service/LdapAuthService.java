...
public interface LdapAuthService {
  ...
  /**
   * Authenticates a user against the LDAP directory. Connects to the LDAP server, authenticates the
   * user, and retrieves user attributes to create or update the local user record.
   *
  ...
  /**
   * Processes an LDAP login request and generates an authentication token. Authenticates the user
   * via LDAP and generates a JWT token for the session.
   *
