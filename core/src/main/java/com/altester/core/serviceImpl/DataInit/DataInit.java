...
public class DataInit {
  ...
  public void createStudentGroups() {
    List<User> students =
        userRepository.findByRole(RolesEnum.STUDENT, Pageable.unpaged()).getContent();
    List<User> teachers =
        userRepository.findByRole(RolesEnum.TEACHER, Pageable.unpaged()).getContent();
    ...
    Semester twoSemestersAgo =
        (previousSemester == Semester.WINTER) ? Semester.SUMMER : Semester.WINTER;
    int twoSemestersAgoYear =
        (twoSemestersAgo == Semester.SUMMER) ? previousYear - 1 : previousYear;
    ...
    Semester twoSemestersAhead =
        (nextSemester == Semester.WINTER) ? Semester.SUMMER : Semester.WINTER;
    int twoSemestersAheadYear = (twoSemestersAhead == Semester.SUMMER) ? nextYear : nextYear + 1;
    createSemesterGroups(twoSemestersAhead, twoSemestersAheadYear, 8, false, students, teachers);
  }

  protected void createSemesterGroups(
      Semester semester,
      int academicYear,
      int groupCount,
      boolean isActive,
    ...
    for (Map.Entry<Subject, Integer> entry : subjectGroupCounts.entrySet()) {
      ...
      for (int i = 0; i < groupsForSubject; i++) {
        String groupName =
            generateGroupName(groupIndex, semester, academicYear, i, subject.getShortName());
        ...
        User teacher =
            teachers.get((int) (subject.getId() % teachers.size() + i) % teachers.size());
        ...
        while (groupStudents.size() < studentCount && attemptsCount < maxAttempts) {
          ...
          Set<User> assignedStudents =
              studentsAssignedToSubject.computeIfAbsent(subject, k -> new HashSet<>());
        ...
        log.info(
            "Created group: {} with {} students for {}/{} - {}",
            groupName,
            groupStudents.size(),
            semester,
            academicYear,
  ...
  protected void createTestsForGroup(Group group, Subject subject) {
    ...
    for (int i = 0; i < testCount; i++) {
      ...
      log.info(
          ...
          group.isActive()
              ? "active"
              : (groupActivityService.isGroupInFuture(group) ? "future" : "past"));
    }
  }
  ...
  private void createQuestionsWithDifficulty(
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (difficulty == QuestionDifficulty.EASY) {
        questionType =
            random.nextDouble() < 0.7 ? QuestionType.MULTIPLE_CHOICE : QuestionType.TEXT_ONLY;
      } else if (difficulty == QuestionDifficulty.MEDIUM) {
        questionType =
            random.nextDouble() < 0.5 ? QuestionType.MULTIPLE_CHOICE : QuestionType.TEXT_ONLY;
      } else {
        questionType =
            random.nextDouble() < 0.3 ? QuestionType.MULTIPLE_CHOICE : QuestionType.TEXT_ONLY;
      }
      ...
      log.debug("Created {} {} question for test '{}'", difficulty, questionType, test.getTitle());
    }
  }
  ...
  private void createProgrammingOptions(
  ...
  protected void createGenericOptions(Question question, int optionCount) {
    ...
    if (difficulty == QuestionDifficulty.EASY) {
      ...
      options.add(
          "The " + baseTerm + " offers better performance but significantly increases complexity");
      options.add(
      ...
      options.add(
          "The " + baseTerm + " is considered best practice only in enterprise-scale applications");
      options.add(
