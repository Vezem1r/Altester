...
public class TestAccessValidator {
  ...
  public void validateTestAccess(User currentUser, Test test) {
    log.debug(
        "Validating test access for user: {}, test ID: {}",
    ...
    if (currentUser.getRole() == RolesEnum.TEACHER) {
      ...
      if (!isTeacherTest) {
        log.warn(
            "Access denied: Teacher {} attempted to access unauthorized test ID: {}",
            currentUser.getUsername(),
            test.getId());
      ...
      log.warn(
          "Access denied: User {} with role {} attempted to access test ID: {}",
          currentUser.getUsername(),
          currentUser.getRole(),
          test.getId());
      throw AccessDeniedException.testAccess();
  ...
  public void validateGroupAccess(User currentUser, Group group) {
    log.debug(
        "Validating group access for user: {}, group ID: {}",
        currentUser.getUsername(),
        group.getId());
    ...
    if (currentUser.getRole() == RolesEnum.TEACHER) {
      ...
      if (!teacherGroups.contains(group)) {
        log.warn(
            "Access denied: Teacher {} attempted to access unauthorized group: {}",
            currentUser.getUsername(),
            group.getName());
      ...
      log.warn(
          "Access denied: User {} with role {} attempted to access group: {}",
          currentUser.getUsername(),
          currentUser.getRole(),
          group.getName());
      throw AccessDeniedException.groupAccess();
  ...
  public boolean hasTestGroupAssociation(User teacher, Test test, List<Group> teacherGroups) {
    log.debug(
        "Checking test-group association for teacher: {}, test ID: {}",
        teacher.getUsername(),
        test.getId());
