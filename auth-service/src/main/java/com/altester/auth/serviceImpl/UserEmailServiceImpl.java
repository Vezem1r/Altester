...
public class UserEmailServiceImpl implements UserEmailService {
    ...
    public void resendMailCode(EmailResendDTO emailResendDTO) {
        log.info("Attempting to resend email verification code for user: {}", emailResendDTO.getUsername());
    ...
    private Codes findEmailChangeCode(User user) {
        return codeRepository.findByUserAndCodeType(user, CodeType.EMAIL_CHANGE)
                .orElseThrow(() -> {
    ...
    private void validateCodeResendTime(Codes code, String email) {
        if (code.getSendAt().plusSeconds(60).isAfter(LocalDateTime.now())) {
            log.warn("Email change code was sent less than a minute ago for user: {}", email);
    ...
    private Codes createEmailChangeCode(User user, String code) {
        ...
        emailCode.setExpiration(LocalDateTime.now().plusMinutes(15));
        return emailCode;
    }
