...
public interface ConversationRepository extends JpaRepository<Conversation, Long> {
    ...
    Optional<Conversation> findConversationBetween(
            @Param("participant1Id") String participant1Id, @Param("participant2Id") String participant2Id);
    ...
    @Query("SELECT c FROM Conversation c WHERE c.participant1Id = :userId OR c.participant2Id = :userId " +
            "ORDER BY c.lastMessageTime DESC")
    Page<Conversation> findConversationsForUserPaginated(@Param("userId") String userId, Pageable pageable);
}