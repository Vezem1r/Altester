...
const AppLayout = ({ children }) => {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        let data;
        if (isAdmin) {
          data = await AdminService.getAdminStats();
          setUserInfo({
            username: data.username || t('appLayoutAdmin', 'Admin'),
            registered: true,
            name: data.name || t('appLayoutAdmin', 'Admin'),
            surname: data.surname || '',
            email: data.email || '',
          });
        } else {
          data = await TeacherService.getTeacherPage();
          setUserInfo({
            username: data.username || t('appLayoutTeacher', 'Teacher'),
            registered: data.registered || false,
            name: data.name || t('appLayoutTeacher', 'Teacher'),
            surname: data.surname || t('appLayoutSuper', 'Super'),
            email: data.email || '',
          });
        }
      } catch {
  ...
  const Header = isAdmin ? AdminHeader : TeacherHeader;
        ...
        <div className="flex flex-col w-0 flex-1 overflow-hidden">
          <div className="md:hidden pl-1 pt-1 sm:pl-3 sm:pt-3 bg-white shadow-sm">
            <button
              ...
              aria-label={t('appLayoutMenu', 'Menu')}
            >
              <svg
                className="h-6 w-6"
              ...
              <div className="fixed inset-0 z-40 flex">
                <div className="fixed inset-0">
                  <button
                    ...
                    aria-label={t('appLayoutCloseSidebar', 'Close sidebar')}
                  />
                </div>
                  ...
                  <div className="pt-2 pb-4">
                    <div className="flex items-center px-4 justify-end">
                      <button
                        ...
                        aria-label={t('appLayoutClose', 'Close')}
                      >
                        <svg
