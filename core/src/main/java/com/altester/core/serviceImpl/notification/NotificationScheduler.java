...
public class NotificationScheduler {
  ...
  public void sendWeeklyStatistics() {
    ...
    int activeUsers = userRepository.countByLastLoginAfter(LocalDateTime.now().minusDays(7));
    int submissions = attemptRepository.countByEndTimeAfter(LocalDateTime.now().minusDays(7));
    ...
    notificationService.notifyUsageStatistics(admins, activeTests, activeUsers, submissions);
  ...
  public void checkTestsWithoutQuestions() {
    try {
      ...
      log.info(
          "Sent notifications about {} tests without questions to {} administrators",
          testsWithoutQuestions.size(),
          admins.size());
    } catch (Exception e) {
