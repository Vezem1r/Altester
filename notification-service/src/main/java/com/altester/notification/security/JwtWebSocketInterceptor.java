...
public class JwtWebSocketInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(@NonNull Message<?> message, @NonNull MessageChannel channel) {
        ...
        if (StompCommand.DISCONNECT.equals(command)) {
            log.debug("Processing DISCONNECT command for user: {}",
                    accessor.getUser() != null ? accessor.getUser().getName() : "unknown");
            return message;
        }

        if (accessor.getUser() == null) {
            log.debug("No user authentication in session for command: {} - attempting to authenticate", command);
            return authenticateUser(message, accessor);
        }

        log.debug("User already authenticated for command {}: {}", command, accessor.getUser().getName());
        return message;
    }
    ...
    private Message<?> authenticateUser(Message<?> message, StompHeaderAccessor accessor) {
        ...
        try {
            ...
            log.info("WebSocket connection authenticated for user: {} with command: {}",
                    username, accessor.getCommand());
        } catch (AuthenticationException e) {
            throw e;
        } catch (Exception e) {
            log.error("WebSocket authentication error for command {}: {}",
