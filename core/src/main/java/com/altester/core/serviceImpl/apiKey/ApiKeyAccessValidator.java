...
public class ApiKeyAccessValidator {
    ...
    /**
     * Validates that the current user has permission to perform the specified operation on the API key.
     *
    ...
    public void validateGroupAccess(Group group, User user, Test test) {
        if (!group.getTests().contains(test)) {
            throw new StateConflictException("group", "mismatch",
                    "The specified group is not associated with this test");
        }
