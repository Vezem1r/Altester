...
public class TestAttemptController {
    ...
    @PostMapping("/start")
    public ResponseEntity<SingleQuestionResponse> startAttempt(Principal principal, @RequestBody StartAttemptRequest request) {
    ...
    public ResponseEntity<Void> saveAnswer(Principal principal, @RequestBody SaveAnswerRequest request) {
        testAttemptService.saveAnswer(principal, request);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/next-question")
    public ResponseEntity<SingleQuestionResponse> nextQuestion(Principal principal, @RequestBody NextQuestionRequest request) {
        SingleQuestionResponse response = testAttemptService.nextQuestion(principal, request);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/previous-question")
    public ResponseEntity<SingleQuestionResponse> previousQuestion(Principal principal, @RequestBody PreviousQuestionRequest request) {
        SingleQuestionResponse response = testAttemptService.previousQuestion(principal, request);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/complete")
    public ResponseEntity<AttemptResultResponse> completeAttempt(Principal principal, @RequestBody CompleteAttemptRequest request) {
        AttemptResultResponse response = testAttemptService.completeAttempt(principal, request);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/status/{attemptId}")
    public ResponseEntity<AttemptStatusResponse> getAttemptStatus(Principal principal, @PathVariable long attemptId) {
