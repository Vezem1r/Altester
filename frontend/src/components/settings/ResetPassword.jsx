...
const ResetPassword = ({ email: initialEmail, onSuccess }) => {
  ...
  const validateEmailForm = () => {
    ...
    if (!userEmail.trim()) {
      newErrors.email = t('resetPassword.emailRequired', 'Email is required');
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(userEmail)) {
      newErrors.email = t(
        'resetPassword.validEmailRequired',
  ...
  const handleRequestReset = async () => {
    if (validateEmailForm()) {
      try {
        ...
        await PasswordService.requestReset(userEmail);

        toast.success(
          t(
            'resetPassword.codeSent',
        ...
        toast.error(
          error.message ||
            t(
              'resetPassword.failedToSendCode',
  ...
  const handleResetPassword = async () => {
    if (validateResetForm()) {
      try {
        ...
        toast.error(
          error.message ||
            t('resetPassword.failedToReset', 'Failed to reset password')
        );
  ...
  const handleResendCode = async () => {
    ...
    try {
      setIsResending(true);
      await PasswordService.resendResetCode(userEmail);
      toast.success(
        t('resetPassword.codeResent', 'Reset code resent to your email!')
      );
      setResendCooldown(60);
    } catch (error) {
      toast.error(
        error.message ||
          t('resetPassword.failedToResend', 'Failed to resend reset code')
      );
    } finally {
    ...
    <div className="space-y-4">
      <div className="text-center mb-6">
        ...
        <p className="mt-4 text-gray-600">
          {t(
            'resetPassword.enterEmailInstructions',
      ...
      <div>
        ...
        >
          {t('resetPassword.emailAddress', 'Email Address')}
        </label>
        <div className="relative mt-1">
          ...
          <input
            ...
            placeholder={t(
              'resetPassword.enterEmailPlaceholder',
  ...
  const renderVerifyStep = () => (
    <div className="space-y-4">
      <div className="text-center mb-6">
        ...
        <p className="mt-4 text-gray-600">
          {t('resetPassword.codeSentTo', "We've sent a verification code to")}
          <br />
        ...
        <div>
          ...
          <div className="relative mt-1">
            ...
            <input
              ...
              placeholder={t(
                'resetPassword.enterCodePlaceholder',
                'Enter 6-digit code'
              )}
              value={form.code}
              onChange={e => {
            ...
            <div className="mt-2 space-y-1">
              <div className="flex items-center">
                ...
                <span className="text-xs text-gray-600">
                  {t('resetPassword.atLeastChars', 'At least 8 characters')}
                </span>
              </div>
              <div className="flex items-center">
                <div
                ...
                <span className="text-xs text-gray-600">
                  {t(
                    'resetPassword.atLeastUppercase',
              ...
              <div className="flex items-center">
                ...
                />
                <span className="text-xs text-gray-600">
                  {t('resetPassword.atLeastNumber', 'At least 1 number')}
                </span>
              </div>
            </div>
          )}
    ...
    <div className="flex flex-col h-full justify-between">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-gray-700 text-center">
          {currentStep === 'request'
            ? t('resetPassword.changePassword', 'CHANGE PASSWORD')
            : t('resetPassword.resetYourPassword', 'RESET YOUR PASSWORD')}
        </h2>
      </div>
        ...
        {currentStep === 'request' ? (
          <div className="flex space-x-2">
            <button
              ...
              disabled={loading}
              className={`w-full py-3 px-4 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-full transition-colors duration-200 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              {loading
                ? t('resetPassword.sendingLoading', 'SENDING...')
                : t(
