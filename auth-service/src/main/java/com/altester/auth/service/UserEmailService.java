...
public interface UserEmailService {
  ...
  /**
   * Initiates the email change process for a user. Validates user credentials, generates a
   * verification code, and sends it to the new email address.
   *
  ...
  /**
   * Resends the email verification code to the specified email address. Generates a new
   * verification code and sends it if enough time has passed since the previous code was sent.
   *
  ...
  /**
   * Completes the email change process using the verification code. Validates the provided code and
   * updates the user's email if valid.
   *
   * @param emailConfirmDTO Data transfer object containing user ID, new email, and verification
   *     code
   * @throws UserNotFoundException if user doesn't exist
