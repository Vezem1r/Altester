...
public class QuestionValidator {
  ...
  public void validateQuestionData(
      QuestionType questionType, String questionText, boolean hasImage, List<OptionDTO> options) {
    ...
    log.debug(
        "Validating question data - type: {}, hasText: {}, hasImage: {}, hasOptions: {}",
        questionType,
        hasText,
        hasImage,
        hasOptions);
  ...
  private void validateTextOnly(boolean hasText, boolean hasImage) {
    if (!hasText) {
      throw ValidationException.invalidQuestionText(
          "TEXT_ONLY question type requires non-empty text");
    }
    if (hasImage) {
  ...
  private void validateImageOnly(boolean hasText, boolean hasImage) {
    ...
    if (hasText) {
      throw ValidationException.invalidQuestionText(
          "IMAGE_ONLY question type should not have text");
    }
  }
  ...
  private void validateTextWithImage(boolean hasText, boolean hasImage) {
    if (!hasText) {
      throw ValidationException.invalidQuestionText(
          "TEXT_WITH_IMAGE question type requires non-empty text");
    }
    if (!hasImage) {
  ...
  private void validateMultipleChoice(
      boolean hasText, boolean hasOptions, List<OptionDTO> options) {
    if (!hasText) {
      throw ValidationException.invalidQuestionText(
          "MULTIPLE_CHOICE question type requires non-empty text");
    }
    if (!hasOptions) {
      throw ValidationException.invalidOption(
          "MULTIPLE_CHOICE question type requires at least one option");
    }

    validateOptionsHaveCorrectAnswer(options);
  }

  private void validateImageWithMultipleChoice(
      boolean hasImage, boolean hasOptions, List<OptionDTO> options) {
    if (!hasImage) {
      throw ValidationException.invalidImage(
          "IMAGE_WITH_MULTIPLE_CHOICE question type requires an image");
    }
    if (!hasOptions) {
      throw ValidationException.invalidOption(
          "IMAGE_WITH_MULTIPLE_CHOICE question type requires at least one option");
    }
  ...
  private void validateOptionsHaveCorrectAnswer(List<OptionDTO> options) {
    ...
    if (!hasCorrectOption) {
      throw ValidationException.missingCorrectOption(
