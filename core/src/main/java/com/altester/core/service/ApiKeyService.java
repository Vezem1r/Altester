...
public interface ApiKeyService {
  ...
  /**
   * Create a new API key - Admin: can create global or non-global keys - Teacher: can only create
   * non-global keys (owned by themselves)
   *
   * @param request The API key creation request containing name, key value, service name, and
   *     global status
   * @param principal The authenticated user
  ...
  /**
   * Delete an API key - Admin: can delete any API key - Teacher: can only delete their own
   * (non-global) API keys
   *
  ...
  /**
   * Update an API key's properties - Admin: can update any API key - Teacher: can only update their
   * own keys
   *
  ...
  /**
   * Toggle activation status of an API key - Admin: can toggle any API key - Teacher: can only
   * toggle their own keys - When deactivating, the key is automatically unassigned from all tests
   *
  ...
  /**
   * Assign an API key to a test for a specific group - Admin: must specify a group ID - Teacher:
   * can assign to a specific group or to all their groups if groupId not specified - Teachers can
   * only assign global keys or their own keys
   *
   ...
   * @throws AccessDeniedException if the user doesn't have permission to access the API key or
   *     group
  ...
  /**
   * Unassign API key from a test for a specific group - Admin: must specify a group ID - Teacher:
   * can unassign from a specific group or from all their groups if groupId not specified
   *
