...
public class CacheablePage<T> implements Page<T>, Serializable {
  ...
  @JsonIgnore private transient Pageable pageable;
  ...
  public CacheablePage(
      List<T> content, int pageNumber, int pageSize, List<SortOrder> sortOrders, long total) {
  ...
  public Pageable getPageable() {
    if (this.pageable == null) {
      this.pageable = PageRequest.of(pageNumber, pageSize > 0 ? pageSize : 10, buildSort());
    }
    ...
    return Sort.by(
        sortOrders.stream()
            .map(
                order ->
                    new Sort.Order(
                        Sort.Direction.valueOf(order.getDirection()), order.getProperty()))
            .toList());
  }
  ...
  public Sort getSort() {
    ...
    for (SortOrder sortOrder : sortOrders) {
      orders.add(
          new Sort.Order(
              Sort.Direction.valueOf(sortOrder.getDirection()), sortOrder.getProperty()));
    }
  ...
  public Pageable previousPageable() {
    return hasPrevious()
        ? PageRequest.of(getNumber() - 1, getSize(), getSort())
        : Pageable.unpaged();
  }
