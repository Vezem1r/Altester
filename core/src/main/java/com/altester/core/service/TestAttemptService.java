...
public interface TestAttemptService {
  ...
  /**
   * Starts a new test attempt or resumes an existing active attempt. If there's an active attempt,
   * continues from where the student left off. If there's no active attempt, creates a new one if
   * the student hasn't exceeded the maximum attempts.
   *
   ...
   * @throws ValidationException if the test is not available for the student or max attempts
   *     reached
   */
  ...
  /**
   * Saves a student's answer for a specific question in a test attempt without changing the current
   * question.
   *
   * @param principal The authenticated student
  ...
  /**
   * Completes a test attempt, calculates the score, and returns the result. Automatically grades
  ...
  /**
   * Retrieves the current status of a test attempt, including answered questions, time remaining,
   * and other metadata. Will auto-complete the attempt if it has expired.
   *
   * @param principal The authenticated student
