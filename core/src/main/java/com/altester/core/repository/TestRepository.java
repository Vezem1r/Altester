...
public interface TestRepository extends JpaRepository<Test, Long> {
    ...
    @Query("SELECT DISTINCT t FROM Test t WHERE " +
    ...
    Page<Test> findAllWithFilters(@Param("searchQuery") String searchQuery,
                                  @Param("isActive") Boolean isActive,
                                  Pageable pageable);

    @Query("SELECT DISTINCT t FROM Test t JOIN Group g ON t MEMBER OF g.tests WHERE " +
    ...
    @Query("SELECT DISTINCT t FROM Test t JOIN Group g ON t MEMBER OF g.tests JOIN Subject s ON g MEMBER OF s.groups " +
            ...
            "LOWER(t.description) LIKE LOWER(CONCAT('%', :searchQuery, '%'))) " +
            "AND (:isActive IS NULL OR t.isOpen = :isActive)")
    Page<Test> findBySubjectWithFilters(@Param("subjectId") Long subjectId,
                                        @Param("searchQuery") String searchQuery,
                                        @Param("isActive") Boolean isActive,
                                        Pageable pageable);

    @Query("SELECT DISTINCT t FROM Test t JOIN Group g ON t MEMBER OF g.tests " +
            "WHERE g.id = :groupId " +
            "AND (:searchQuery IS NULL OR :searchQuery = '' OR " +
            "LOWER(t.title) LIKE LOWER(CONCAT('%', :searchQuery, '%')) OR " +
            "LOWER(t.description) LIKE LOWER(CONCAT('%', :searchQuery, '%'))) " +
            "AND (:isActive IS NULL OR t.isOpen = :isActive)")
    Page<Test> findByGroupWithFilters(@Param("groupId") Long groupId,
                                      @Param("searchQuery") String searchQuery,
                                      @Param("isActive") Boolean isActive,
                                      Pageable pageable);
}