...
public interface StudentService {
  ...
  /**
   * Retrieves the student dashboard containing current active groups and their tests, with optional
   * filtering by search query and/or specific group.
   *
   ...
   * @throws AccessDeniedException if the student attempts to access a group they are not a member
   *     of
   */
  ...
  /**
   * Retrieves the student's academic history grouped by academic periods (semester and year), with
   * optional filtering by specific period and/or search query. Only includes past (inactive) groups
   ...
   * @throws ResourceNotFoundException if the student is not found
   */
  AcademicHistoryResponse getAcademicHistory(
      Principal principal, Integer academicYear, Semester semester, String searchQuery);

  /**
   * Retrieves the list of all academic periods (semester and year combinations) available in the
   * student's academic history, sorted by recency.
   *
   * @param principal The authenticated student
  ...
  /**
   * Retrieves detailed review information for a specific test attempt, including questions,
